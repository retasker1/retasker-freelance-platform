      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white shadow rounded-lg p-6 mb-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">
            Отклики ({order.deals?.length || 0})
          </h2>
          
          {order.deals && order.deals.length > 0 ? (
            <div className="space-y-4">
              {order.deals.map((deal: any) => (
                <div key={deal.id} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center">
                      {deal.freelancer.photoUrl && (
                        <img 
                          src={deal.freelancer.photoUrl} 
                          alt={deal.freelancer.firstName}
                          className="w-8 h-8 rounded-full mr-3"
                        />
                      )}
                      <div>
                        <p className="text-sm font-medium text-gray-900">
                          {deal.freelancer.firstName} {deal.freelancer.lastName || ''}
                        </p>
                        {deal.freelancer.username && (
                          <p className="text-sm text-gray-500">@{deal.freelancer.username}</p>
                        )}
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-lg font-semibold text-gray-900">
                        ${(deal.amountCents / 100).toFixed(0)}
                      </p>
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        ...
                        {deal.status === 'PENDING' ? 'Ожидает' :
                         deal.status === 'ACTIVE' ? 'Активна' :
                         deal.status === 'DELIVERED' ? 'Доставлено' :
