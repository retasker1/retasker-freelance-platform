...
import { ordersHandler, myOrdersHandler } from './handlers/orders';
...
const apiService = new ApiService(process.env.APP_API_BASE || 'http://localhost:5173');

// Добавляем сервис API в контекст
bot.use((ctx, next) => {
  (ctx as any).apiService = apiService;
  return next();
});
...
bot.command('my_orders', myOrdersHandler);
...
bot.on('callback_query', async (ctx) => {
  ...
  try {
    ...
    if (data === 'my_deals') {
      ...
      await myOrdersHandler(ctx);
    } else if (data === 'orders') {
      await ordersHandler(ctx);
    } else if (data === 'chat_menu') {
      await chatHandler(ctx);
    } else if (data === 'help') {
      await ctx.reply('🤖 Retasker Bot - Помощь\n\n' +
        'Основные команды:\n' +
        '/start - Начать работу с ботом\n' +
        '/orders - Просмотр открытых заказов\n' +
      ...
      if (botCtx.apiService) {
        ...
        if (deal && botCtx.userId) {
          await (startHandler as any).showDealInfo(ctx, deal, botCtx.userId);
        }
      }
      ...
      (ctx as any).dealId = dealId;
      (ctx as any).waitingForMessage = true;
